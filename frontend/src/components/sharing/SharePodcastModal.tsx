...
interface SharePodcastModalProps {
  podcastId: string;
  podcastTitle: string;
  onClose: () => void;
}
...
export default function SharePodcastModal({ podcastId, podcastTitle, onClose }: SharePodcastModalProps) {
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
          <h3 className="font-mono font-medium text-text-primary" style={{ fontSize: 16 }}>Share Podcast</h3>
          <button
            onClick={onClose}
        ...
        >
          <Send size={15} />
          {shareMutation.isPending ? 'Sharing...' : 'Share Podcast'}
        </button>
